...
export default function ReportTable({ onMonthlyBalance }) {
  ...
  useEffect(() => {
    if (selectedMonth && allReports.length > 0) {
      const filteredReports = allReports.filter(report => report.month === selectedMonth);
  ...
  const monthlyBalance = openingBalance + totals.cash + totals.card - totals.market;
  ...
  if (!allReports.length) {
    return (
      <div className="text-center py-8">
        <div className="text-gray-400 mb-2">📊</div>
        <p className="text-gray-400 mb-4">No records found</p>
        <button className="text-[var(--color-gold)] border border-[var(--color-gold)]/30 rounded-lg px-4 py-2 text-sm">
          Add First Record
        </button>
      </div>
    ...
    <div className="mb-6">
      ...
      {availableMonths.length > 0 && (
        <div className="mb-4 flex items-center gap-3">
          ...
          <select
            ...
            className="bg-[var(--color-panel)] border border-white/20 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-[var(--color-gold)]"
          >
      ...
      {reports.length === 0 ? (
        <div className="text-center text-gray-400 py-10 border border-white/10 rounded-xl">
          No records for {selectedMonth ? formatMonthDisplay(selectedMonth) : 'this month'}
        </div>
      ) : (
        <div className="bg-[var(--color-panel)] border border-white/10 rounded-xl overflow-hidden">
              ...
              <table className="w-full text-sm">
                ...
                <thead className="bg-white/5">
                  <tr className="text-xs">
                    ...
                    <th className="p-3 text-right font-medium text-gray-300">CASH</th>
                    <th className="p-3 text-right font-medium text-gray-300">MARKET</th>
                    <th className="p-3 text-right font-medium text-gray-300">CARD</th>
                    <th className="p-3 text-right font-medium text-gray-300">EXPENSE</th>
                    <th className="p-3 text-right font-medium text-gray-300">DAILY TOTAL</th>
                  </tr>
                </thead>
                  ...
                  {reports.map((r, i) => {
                    ...
                    return (
                      <tr key={i} className="hover:bg-white/5 transition-colors">
                        ...
                        <td className="p-3 text-right">
                          <span className={`font-medium ${r.cash > 0 ? "text-green-400" : "text-gray-400"}`}>
                            ₹{r.cash.toLocaleString()}
                          </span>
                        </td>
                        <td className="p-3 text-right">
                          <span className={`font-medium ${r.market > 0 ? "text-amber-400" : "text-gray-400"}`}>
                            ₹{r.market.toLocaleString()}
                          </span>
                        </td>
                        <td className="p-3 text-right">
                          <span className={`font-medium ${r.card > 0 ? "text-blue-400" : "text-gray-400"}`}>
                            ₹{r.card.toLocaleString()}
                          </span>
                        </td>
                        <td className="p-3 text-right">
                          <span className={`font-medium ${r.expense > 0 ? "text-red-400" : "text-gray-400"}`}>
                            ₹{r.expense.toLocaleString()}
                          </span>
                        </td>
                        <td className="p-3 text-right">
                ...
                <tfoot>
                  ...
                  <tr className="bg-white/10">
                    ...
                    <td className="p-3 text-right">
                      ...
                      <div className="text-xs text-gray-400 mt-1">Market Total</div>
                    </td>
                    <td className="p-3 text-right">
                      <div className="font-bold text-blue-400">
                        ₹{totals.card.toLocaleString()}
                    ...
                    <td className="p-3 text-right">
                      ...
                      <div className="text-xs text-gray-400 mt-1">Expense Total</div>
                    </td>
                    <td className="p-3 text-right">
                      <div className="font-bold text-[var(--color-gold)]">
                        ₹{totals.dailyTotal.toLocaleString()}
                      </div>
                      <div className="text-xs text-gray-400 mt-1">Daily Total</div>
                    </td>
                  </tr>
                </tfoot>
              </table>
